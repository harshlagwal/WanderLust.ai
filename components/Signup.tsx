...
const Signup: React.FC<SignupProps> = ({ onSwitch }) => {
    ...
    const handleSignup = async (e: React.FormEvent) => {
        ...
        try {
            console.log('[SIGNUP COMPONENT] Submitting signup form...');
            await signup(name, email, password);
            console.log('[SIGNUP COMPONENT] Signup successful, AuthContext should now trigger redirect.');
        } catch (err: any) {
            console.error('[SIGNUP COMPONENT] Signup submission error:', err);
            if (!authError) {
                setLocalError(err.response?.data?.message || err.message || 'Failed to sign up');
            }
        } finally {
            setLoading(false);
        }
    ...
    return (
        <div className="bg-white/10 backdrop-blur-md p-8 rounded-2xl border border-white/20 shadow-2xl w-full max-w-md animate-fade-in text-white">
            <h2 className="text-3xl font-bold mb-6 text-center">Join Wanderlust</h2>
            ...
            <p className="text-white/60 text-center mt-8 text-sm">
                Already have an account? <button onClick={onSwitch} className="text-blue-400 hover:underline">Login</button>
            </p>
        </div>
    );
};
